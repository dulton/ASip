...
// Copyright (C) ANPI LLC, USA
// This file is for ANPI LLC Diagnosis tool
// Since pjsip open source library is used in accorance the 
// licence for this or other files including binary library files 
// are as stated above paragraphs.

#include "stdafx.h"
#include "LanTool.h"
extern "C" {

struct TOOLDATA ToolData;

static void print_call_log(int call_index); // Subbu

static char *DLL_call_log(int call_index); // Subbu
				   ...
				   pjsip_event *e)
{
	...
	if (app.call_report) {
	    ...
	    // print_call_log(call->index);
		// print_call(call->index);
		DLL_call_log(call->index);
	}
	...
	else if (inv->state == PJSIP_INV_STATE_CONFIRMED) 
	{
		...
		PJ_LOG(3,(THIS_FILE, "Call #%d connected in %d ms", call->index +1 ,
		  PJ_TIME_VAL_MSEC(t)));
...
static pj_status_t init_options(int argc, char *argv[], char* Name)
{
...
*/
    ...
    pj_ansi_sprintf( local_uri, "sip:%s@%s:%d", Name, app.local_addr.ptr, app.sip_port);
...
#define BIGVAL		    0x7FFFFFFFL
    ...
    sprintf(ToolData.summary,"Total %d call(s) active.\n"
	   "                    Average Statistics\n"
	   "                    min     avg     max \n"
	   "                -----------------------\n"
...
static void print_call(int call_index)
{
	...
	sprintf(duration,  " [duration: %02ld:%02ld:%02ld]",
		now.sec / 3600,
		(now.sec % 3600) / 60,
		(now.sec % 60) );
    ...
    PJ_LOG(3, (THIS_FILE,
	      "Call #%d: To Anpi Diagnostics %s%s", 
	      (call_index +1) , pjsip_inv_state_name(inv->state), 
	      duration));
...
/* Subu END */
/* #include "siprtp_report.c" /* excluded by Subbu */ 
...
static void hangup_call(unsigned index)
{
	...
	;// pjsip_inv_send_msg(app.call[index].inv, tdata); // Subbu commented on 7/21/2014
}
...
static void console_main()
{
  ...
  //  printf("%s", MENU);
    ...
    for (;;) {
	for(index = 0; index < app.max_calls; index++)
			if (app.call[index].inv != NULL)
				program_end = false;
			else
				program_end = true;
...
*/  
	ZeroMemory(&ToolData, sizeof(ToolData));
	if( strlen(argv[1]) > 100 || strlen (argv[1]) < 5) {
		sprintf(ToolData.error, "Error: User_ID should be atleast 5 character length and less than 100 characters"); 
		sprintf(ToolData.error, Using);
		return -1;
	}
	if (validateIP(argv[2]) != VALID){
		sprintf(ToolData.error,Using);
		return -1;
	}
	if ( atoi(argv[3]) > 5 || atoi(argv[3]) < 1){
		sprintf(ToolData.error,"Error: Number of calls should not exceed 5 and at least 1");
		sprintf(ToolData.error, Using);
		return -1;
	}
	
	strcpy(anpi_uri, "sip:Anpi_Diagnosis@");
	strcat(anpi_uri, argv[2]);
	...
	argvs[4] = duration; // -d (duration in seconds)
	argvs[5] = argv[4];
...
//	printf(" %s %s, %s, %s ", argv[0], argvs[1], argvs[2], argvs[3]); // SUBBU commented on July 7/21/2014

	

    /* Get command line options */
    status = init_options(argc, argvs, argv[1]);
    if (status != PJ_SUCCESS)
		return 1;
	...
	sprintf(ToolData.error, "Error: --auto-quit option only valid for outgoing "
	       "mode (UAC) only\n");
	return 1;
    }
    ...
    status = init_sip();
    if (status != PJ_SUCCESS) {
	//sprintf(ToolData.error, THIS_FILE, "Initialization has failed", status);// Subbu commented on 7/21/2014
	destroy_sip();
	return 1;
    }
    ...
    status = init_media();
    if (status != PJ_SUCCESS) {
//	app_perror(THIS_FILE, "Media initialization failed", status); // Subbu commented on July 21, 2014
	destroy_sip();
	return 1;
    }
...
static void print_call_log(int call_index)
{
    struct call *call = &app.call[call_index];
    int len;
    pjsip_inv_session *inv = call->inv;
...
/*
	...
	if (P1 && P2){
		P1 +=4;
		for(index = 0; (P1+index) < P2 && index < 256;  index++)
			user_id[index] = P1[index];
	
		
		user_id[index] = '\0';
		strcat(user_id, ".log");
		printf("\n********** file name is %s**", user_id);
		dlog = fopen(user_id, "a");
		if (dlog){
			fprintf(dlog, "%s\n", userinfo);
		}
	}
...
#define DLLSTORE 1
static char *DLL_call_log(int call_index)
{
    struct call *call = &app.call[call_index];
    int len;
    pjsip_inv_session *inv = call->inv;
...
//	int index;
	...
	char *logstore = (char *) malloc(4096);
	char *logstoredll = (char *) malloc(4096);
...
#if DLLSTORE 
	sprintf(ToolData.call, "\n" THIS_FILE "Call #%d: %s%s", 
	      call_index, pjsip_inv_state_name(inv->state), 
	      duration);
	strcat(ToolData.call, logstore);
...
/* Subbu End */
	...
	if (call->connect_time.sec) {
		...
		sprintf(logstore, THIS_FILE 
		   "   Signaling quality: %s%s", pdd, connectdelay);

		strcat(logstoredll, logstore);
	}
#else
	PJ_LOG(3, (THIS_FILE, 
		   "   Signaling quality: %s%s", pdd, connectdelay));
	...
	printf(" Shri Ganesha, Shri Narasimha\n %s", logstoredll);

	return (char * ) NULL;
}


struct TOOLDATA *WINAPI getStat()
{
	print_avg_stat();
	list_calls();
	return &ToolData;
}
DWORD WINAPI  toolmain(LPVOID lrtpinet){
	anpi_tool(4, (char **) (lrtpinet) );
	return 1;
}


void *WINAPI startSIPSessionint (char *argv[]){
	const int NO_THREADS = 1;
	HANDLE  hThread[NO_THREADS];
	LPDWORD   dwThreadID =0;
	LPDWORD threadstatus = (DWORD *) malloc(sizeof (WORD));
	hThread[0] = CreateThread( NULL,  0, toolmain, (LPVOID) argv, 0, dwThreadID);
	if(hThread[0] == NULL)
		return NULL;
	return getStat;
}

void WINAPI hangup(int n)
{
	hangup_call(n-1);
}

void WINAPI StopSIPSIPSession()
{
	hangup_all_calls();
}
...
int WINAPI checkConnectivity(char *fqdn, int port)
{
	int status;
	char argv[6][100];
	int tcptmain(int argc, char **argv);

	sprintf(argv[0], "tcp Connection");
	sprintf(argv[1], "-s:IP", port);
	sprintf(argv[2], "-p:%s", fqdn);
	sprintf(argv[4], "-n:0");
	sprintf(argv[5], "-o", fqdn);

	status = tcptmain( 6, (char**) argv);
