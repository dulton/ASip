...
*/
	if( strlen(argv[1]) > 100 || strlen (argv[1]) < 5) {
		puts("Error: User_ID should be atleast 5 character length and less than 100 characters"); 
		puts(Using);
		return -1;
	}
	...
	if ( atoi(argv[3]) > 5 || atoi(argv[3]) < 1){
		puts("Error: Number of calls should not exceed 5 and at least 1");
		puts(Using);
		return -1;
	}
	...
	printf(" %s %s, %s, %s ", argv[0], argvs[1], argvs[2], argvs[3]);
